...
import {
  ...
  normalizePrdDraft,
  stringifyAnswer,
} from "./elicitationHelpers.js";

import { clampText, clampInteger } from "../utils/stringUtils.js";
...
export function normalizeQuestionnaireInput(input: unknown): z.infer<typeof QuestionnaireSchema> {
...
export async function readProjectContextLocal(projectRoot: string): Promise<string> {
...
export function buildFallbackQuestionnaire(): ClarificationQuestion[] {
  return [
    {
      id: "Q_CORE_1",
...
export function buildPrdMcpDiagnosis(params: {
...
export async function writeFinalizeDraftFiles(params: {
...
export class ElicitationService {
  async processElicitation(params: {
    requirement: string;
    context: ClarificationContext;
    elicitationMode: "quick" | "deep";
    workspacePath: string;
    batchSize: number;
    targetTotal: number;
    apiKey: string;
